...
gright =  guard(2, vars, loc1, loc2, Xgright, x);
gbot =  guard(3, vars, loc1, loc2, Xgbot, x);
gleft = guard(4, vars, loc1, loc2, Xgleft, x);
...
% parallel = 1;
% 
% % init = struct('init', @init_sampler, 'N', 100);
% init = struct('init', @init_sampler, 'N', 20);
% [osm, osd] = PM.sample_traj_multi(init, 1, parallel);
% PM.plot_nonneg(osd)
% % loc1.dual.gamma
% 
% function [id, x] =  init_sampler()
%     id = randi(2);
%     xraw = 2*rand(2, 1) - 1;
%     if id == 1
%         x = xraw*0.1 + [0.3; 0.3];
